---
name: crawl-wanted
description: 원티드 프론트엔드 5년+ 채용공고를 전체 크롤링하고 스킬 트렌드를 집계합니다.
---

원티드 프론트엔드 5년+ 채용공고를 전체 크롤링하고 스킬 트렌드를 집계합니다.
...
- 전체 JD를 페이징으로 수집 (목록 300ms, 상세 500ms 딜레이)
- 공고 수에 따라 응답 대기 시간이 길 수 있음 (수 분) — 정상
...
```json
{ "ok": true, "added": 150, "skipped": 30, "skillsTracked": 25 }
```
...
- Wanted v4 API (`tag_type_ids=669` = 프론트엔드, `years=5` = 경력 5년+)
- 전체 공고를 offset 페이징으로 수집
